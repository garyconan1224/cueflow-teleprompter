...
项目只保留两个 `.bat` 入口：`setup_portable_env.bat` 负责首次安装当前文件夹内的 `.venv`，`run_portable_app.bat` 负责日常启动。它们都按“脚本所在目录”执行，不依赖固定盘符或固定文件夹名。
...
所有依赖都会安装到当前文件夹下的 `.venv`，不会写进系统 Python，也不会污染全局环境。
